defmodule ADK.Integration.OpenAITest do
  @moduledoc """
  Integration tests for the LiteLlm provider against OpenAI directly.
  ...
  setup do
    ...
    if is_nil(api_key) do
      IO.puts("Skipping OpenAI integration test: OPENAI_API_KEY not set")
      :ok
    end

    name = :"openai_session_#{:erlang.unique_integer([:positive])}"
  ...
  test "simple question", ctx do
    if is_nil(ctx[:api_key]), do: flunk("OPENAI_API_KEY required")

    model = %LiteLlm{model_name: "gpt-4o-mini", api_key: ctx[:api_key]}
    agent = %LlmAgent{name: "openai-agent", model: model, instruction: "Be brief."}
    ...
    events =
      ...
      |> Runner.run(
        "user-1",
        "s1",
        Content.new_from_text("user", "What is 2+2? Reply with just the number.")
      )
      |> Enum.to_list()
  ...
  @tag :integration
  test "tool call with OpenAI", ctx do
    if is_nil(ctx[:api_key]), do: flunk("OPENAI_API_KEY required")
    ...
    events =
      ...
      |> Runner.run(
        "user-1",
        "s1",
        Content.new_from_text("user", "What's the weather in London?")
      )
      |> Enum.to_list()
